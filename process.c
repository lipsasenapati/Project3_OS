...
void context_in( pid) {
	...
	CPU.CPU_sockfd = PCB[pid]->PCB_sockfd; //"CHANGED"
}
...
void context_out( pid) {
	...
	PCB[pid]->PCB_sockfd = CPU.CPU_sockfd;
...
void insert_ready_process(pid)
	int pid; {
	ReadyNode *node;
	printf("Hello from insert_ready_process \n");
...
void end_process(pid)
	...
	{
		sprintf(str, "Process had completed successfully!!!");
		printf("Helloooooooo %s\n", str);
		send_client_result(PCB[pid]->PCB_sockfd, str);
	}
...
void submit_process(fname, sockfd)
	...
	for (i = 0; i < numinstr; i++) {
		fscanf(fprog, "%d %d\n", &opcode, &operand);
		if (Debug)
			printf("Process %d load instruction: %d, %d, %d\n", pid, i, opcode,
					operand);
	...
	for (i = 0; i < numdata; i++) {
		...
		if (Debug)
			printf("Process %d load data: %d, %.2f\n", pid, i, data);
		if (ret == mError) {
			PCB[pid]->exeStatus = eError;
			return;
		}
	}
	printf("Hello....in sub_process\n");
